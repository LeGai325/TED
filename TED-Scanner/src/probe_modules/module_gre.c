...
int gre_global_initialize(struct state_conf *conf)
{
	return EXIT_SUCCESS;
}

static int gre_init_perthread(void *buf, macaddr_t *src,
...
static int gre_make_packet(void *buf, size_t *buf_len,
				     ...
				     UNUSED int probe_num, UNUSED void *arg)
{
	...
	ip_header2->ip_dst = ((struct in_addr *)arg)[0];// External IP address
...
static void gre_print_packet(FILE *fp, void *packet)
{
...
static int gre_validate_packet(const struct ip *ip_hdr, uint32_t len,
...
static void gre_process_packet(const u_char *packet,
...
probe_module_t module_gre = {
    .name = "gre",
    .max_packet_length = sizeof(struct ether_header) + sizeof(struct ip) + sizeof(gre_header_default) +
			 sizeof(struct ip) + sizeof(struct icmp),
    ...
    .port_args = 0,
    .global_initialize = &gre_global_initialize,
    .thread_initialize = &gre_init_perthread,
    .make_packet = &gre_make_packet,
    .print_packet = &gre_print_packet,
    .process_packet = &gre_process_packet,
    .validate_packet = &gre_validate_packet,
    .close = NULL,
    .fields = fields,
    .numfields = 6};