...
probe_module_t module_gre6_icmp;
...
int gre6_icmp_global_initialize(struct state_conf *conf)
{
	...
	if (asprintf((char **restrict)&module_gre6_icmp.pcap_filter,
		     "%s && ip6 dst host %s", module_gre6_icmp.pcap_filter,
...
static int gre6_icmp_init_perthread(void *buf, macaddr_t *src, macaddr_t *gw,
			       ...
			       __attribute__((unused)) void **arg_ptr)
{
	...
	uint16_t payload_len =
	    sizeof(gre_header_t) + sizeof(struct ip6_hdr) +
...
static int gre6_icmp_make_packet(void *buf, size_t *buf_len,
			    UNUSED ipaddr_n_t src_ip, UNUSED ipaddr_n_t dst_ip,
...
static void gre6_icmp_print_packet(FILE *fp, void *packet)
{
...
static int gre6_icmp_validate_packet(const struct ip *ip_hdr, uint32_t len,
...
static void gre6_icmp_process_packet(const u_char *packet,
...
probe_module_t module_gre6_icmp= {
    ...
    .port_args = 0,
    .global_initialize = &gre6_icmp_global_initialize,
    .thread_initialize = &gre6_icmp_init_perthread,
    .make_packet = &gre6_icmp_make_packet,
    .print_packet = &gre6_icmp_print_packet,
    .process_packet = &gre6_icmp_process_packet,
    .validate_packet = &gre6_icmp_validate_packet,
    .close = NULL,
    .fields = fields,
    .numfields = 7};
