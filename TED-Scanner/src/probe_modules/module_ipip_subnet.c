...
probe_module_t module_ipip_subnet;

int ipip_subnet_global_initialize(struct state_conf *conf)
{
	return EXIT_SUCCESS;
}

static int ipip_subnet_init_perthread(void *buf, macaddr_t *src,
...
ipaddr_n_t get_subnet_address_ipip(ipaddr_n_t decimal_address){
...
static int ipip_subnet_make_packet(void *buf, size_t *buf_len,
				     ...
				     UNUSED int probe_num, UNUSED void *arg)
{
	...
	ip_header2->ip_src.s_addr = get_subnet_address_ipip(dst_ip);
...
static void ipip_subnet_print_packet(FILE *fp, void *packet)
{
...
static int ipip_subnet_validate_packet(const struct ip *ip_hdr, uint32_t len,
...
static void ipip_subnet_process_packet(const u_char *packet,
...
probe_module_t module_ipip_subnet = {
    ...
    .port_args = 0,
    .global_initialize = &ipip_subnet_global_initialize,
    .thread_initialize = &ipip_subnet_init_perthread,
    .make_packet = &ipip_subnet_make_packet,
    .print_packet = &ipip_subnet_print_packet,
    .process_packet = &ipip_subnet_process_packet,
    .validate_packet = &ipip_subnet_validate_packet,
    .close = NULL,
    .fields = fields,
    .numfields = 7};