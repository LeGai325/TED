...
probe_module_t module_ipip_ttl;

int ipip_ttl_global_initialize(struct state_conf *conf) 
{
	if(!zconf.spoofing_address_v4){
		log_error("ipip_ttl", "Spoofing IPv4 address was not given. Add --spoofing-address-v4");
		return EXIT_FAILURE;
	}
	return EXIT_SUCCESS;
}

static int ipip_ttl_init_perthread(void *buf, macaddr_t *src, macaddr_t *gw,
				   ...
				   UNUSED void **arg_ptr)
{
	...
	uint16_t payload_len =
	    htons(sizeof(struct ip) + sizeof(struct ip) + sizeof(struct icmp));
...
static int ipip_ttl_make_packet(void *buf, size_t *buf_len, ipaddr_n_t src_ip,
				...
				UNUSED void *arg)
{
	...
	ip_header2->ip_dst = ((struct in_addr *)arg)[0]; //Random Spoofed address
	ip_header2->ip_src = ((struct in_addr *)arg)[1]; // external_ip
...
static void ipip_ttl_print_packet(FILE *fp, void *packet)
{
...
static int ipip_ttl_validate_packet(const struct ip *ip_hdr, uint32_t len,
...
static void ipip_ttl_process_packet(const u_char *packet,
...
probe_module_t module_ipip_ttl = {
    ...
    .pcap_filter = "icmp[0]==11", // and icmp6[0]=!8",
    .pcap_snaplen =
	118,
    .port_args = 0,
    .global_initialize = &ipip_ttl_global_initialize,
    .thread_initialize = &ipip_ttl_init_perthread,
    .make_packet = &ipip_ttl_make_packet,
    .print_packet = &ipip_ttl_print_packet,
    .process_packet = &ipip_ttl_process_packet,
    .validate_packet = &ipip_ttl_validate_packet,
    .close = NULL,
    .fields = fields,
    .numfields = 7};