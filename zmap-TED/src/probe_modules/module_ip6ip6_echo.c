...
probe_module_t module_ip6ip6_echo;

int ip6ip6_echo_global_initialize(struct state_conf *conf)
{
	// Only look at received packets destined to the specified scanning address (useful for parallel zmap scans)
	if (asprintf((char **restrict)&module_ip6ip6_echo.pcap_filter,
		     "%s && ip6 dst host %s", module_ip6ip6_echo.pcap_filter,
...
static int ip6ip6_echo_init_perthread(void *buf, macaddr_t *src, macaddr_t *gw,
				      ...
				      __attribute__((unused)) void **arg_ptr)
{
	...
	uint16_t payload_len = sizeof(struct ip6_hdr) +
			       sizeof(struct icmp6_hdr) + 2 * sizeof(uint32_t);
	make_ip6_header(ip6_header, IPPROTO_IPV6, payload_len);

	struct ip6_hdr *ip6_header2 = (struct ip6_hdr *)(&ip6_header[1]);
	// ICMPv6 header plus 8 bytes of data (validation)
	payload_len = sizeof(struct icmp6_hdr) + 2 * sizeof(uint32_t);
...
static int ip6ip6_echo_make_packet(void *buf, size_t *buf_len,
				   ...
				   UNUSED void *arg)
{
	...
	ip6_header2->ip6_dst = ip6_header->ip6_dst;
...
static void ip6ip6_echo_print_packet(FILE *fp, void *packet)
{
...
static int ip6ip6_echo_validate_packet(const struct ip *ip_hdr, uint32_t len,
				       ...
				       uint32_t *validation)
{
	...
	if (icmp6_h->icmp6_type == ICMP6_TIME_EXCEEDED ||
	    icmp6_h->icmp6_type == ICMP6_DST_UNREACH ||
	    icmp6_h->icmp6_type == ICMP6_PACKET_TOO_BIG ||
	    icmp6_h->icmp6_type == ICMP6_PARAM_PROB) {
	...
	if (icmp6_h->icmp6_data32[1] != validation[0] ||
	    icmp6_h->icmp6_data32[2] != validation[1]) {
		return PACKET_INVALID;
	}
	return PACKET_VALID;
}

static void ip6ip6_echo_process_packet(const u_char *packet,
...
probe_module_t module_ip6ip6_echo = {
    ...
    .port_args = 0,
    .global_initialize = &ip6ip6_echo_global_initialize,
    .thread_initialize = &ip6ip6_echo_init_perthread,
    .make_packet = &ip6ip6_echo_make_packet,
    .print_packet = &ip6ip6_echo_print_packet,
    .process_packet = &ip6ip6_echo_process_packet,
    .validate_packet = &ip6ip6_echo_validate_packet,
    .close = NULL,
    .fields = fields,
    .numfields = 7};
